...
Individual-gene cluster-bootstrap intervals were not recalculated because individual-gene scores for every biological profile are not included. Such intervals are mathematically defined for a one-gene set; their absence here is a data-availability limitation. The Figure 6 conditional intervals remain unchanged. Two of the six plotted source tables were regenerated by the self-exclusion correction; the other four are unchanged.

## Effect of the background self-exclusion correction

The one-gene bands are computed from the background scores and therefore moved when every scored gene was excluded from its own control pool on 21 September 2026; the two-gene band moved with the size-two band of the main figure. No classification in this analysis changed, under either reference method. The limits quoted above are the corrected ones, and the released-versus-corrected comparison for all twelve rows is in audit/self_exclusion_2026-09-21/sensitivity_changes.tsv.

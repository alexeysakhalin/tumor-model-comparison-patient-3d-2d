...
Print-resolution panels, each drawn by the same panel function as the composite:

```bash
python code/export_panels.py tables figures/panels 600
```

The panels ship in `figures/panels/`; the export rewrites them and `figures/panels/PANEL_TEXT_OVERLAPS.tsv`, which records every touching pair of text
boxes per panel; all three panels are collision-free at the sizes configured in the script.
...
## Correction of background self-matching (1.1.0)

Every scored gene is now excluded from its own control pool. The 200 background genes could previously
select themselves at distance zero, which propagated into the reference bands and the tables normalised
by them; the analysed estimates and their intervals were never affected and are verified unchanged. The
released baseline was reproduced before the difference was measured:
5 of 24 normalised contrasts change, each by at most
0.04 reference-band units, all 48 bands widen, and no classification in
Figure 6 changes. `CHANGELOG.md` lists the affected files and
`audit/self_exclusion_2026-09-21/Background_Self_Matching_Correction_Report_EN.md` documents the defect,
the reproduction of the baseline and the measured effect.

The two correction utilities are not part of the default workflow, because they need the primary
matrices and the previously released tables:

```bash
python code/audit_self_exclusion.py pipeline.conf --tables <released tables> --output-dir audit/run
python code/report_self_exclusion.py --released <released tables> --corrected tables --output-dir audit/run
```

The selection rule itself is covered by `code/test_numerics.py`, which needs no primary data.
...
`ZENODO_TABLE_SNAPSHOT.json` records the SHA-256 checksum of each of the 51 numerical and provenance tables and names the source-data archive built from them, `Figure6_Source_Data_v1.1.0.zip`, with that archive's own checksum. Eleven of the 51 tables were regenerated by the self-exclusion correction, so they are no longer byte-identical to the v1.0.0 archive; `CHANGELOG.md` lists them. The associated data-record DOI is [10.5281/zenodo.22863363](https://doi.org/10.5281/zenodo.22863363); the record remained unpublished at the time of this repository check. This repository does not assign that data DOI to its software.

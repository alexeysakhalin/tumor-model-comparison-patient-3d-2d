...
The control-selection utility excludes every scored gene from its own control pool, background genes included. The earlier rule excluded only the analyzed genes, so a background gene could take itself as its nearest control. The correction was reconstructed by the supplying analyst from the original expression matrices, not from the deposited group scores; with the deposited files alone a reader can verify the corrected tables against each other and the two control-assignment sets against each other, but not that reconstruction: `code/audit_self_exclusion.py` rebuilds the rank matrices, scores the background genes under both rules in one process, reproduces the released background table before reporting any difference, and records the inputs it used with sizes and checksums in `audit/self_exclusion_2026-09-21/input_manifest.tsv` (127 files, enumerated through the same selection rules the loaders use rather than by directory glob, with logical paths that carry no local directory of the preparing machine). Those matrices are still not redistributed here; the manifest and this document identify them.
